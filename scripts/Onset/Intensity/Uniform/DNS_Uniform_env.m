%% check how the tones are distributed for the envelope 
OT_setup 
...
for s = 1:length(sbj)
    for k = 1:2
        part_sum = cat(1,part_sum,envs{s,k});
...
for s=1:length(sbj)
    ...
    for k=1:2
        ...
        if size(resp,1)>length( menv)
            resp = resp(1:length( menv),:);
        elseif size(resp,1)<length( menv)
             menv =  menv(:,1:size(resp,1));
        end
        ...
        for i = 1:length(numBins)
            ...
            %go for the envelope
            %binned envelope
            [counts, ~, binIndices] = histcounts(menv_norm, binEdges{i});

            env_bin = zeros(length(counts),length(menv));
            
            for io = 1:length(binIndices)
                env_bin(binIndices(io),io) = menv_norm(io);
            end
            ...
            %% compute the percentage of tones in each bin
            ovlp = [];
            for n = 1:size(stim,2)
                ...
                for t = 1:size(tone,2)
                    ...
                    %find the true env value 
                    tone_bin = [];
                    for o = 1:length(tone_idx)
                        %max value or mean value?
                        if tone_idx(o)+5 < EEG.pnts
                            tone_bin(o) = max(binIndices(tone_idx(o)-1:tone_idx(o)+5));
                        else
                            tone_bin(1,o) = binIndices(tone_idx(o));
                        end
                    end
                    
                    ovlp(n,t) = (sum(tone_bin == n)./length(tone_idx))*100
...
save('DNS_Intensity_uniformbins_env.mat','-struct','DNS_Intensity')
...
%     title(t,'DNS based TRF','FontSize',28)
    ...
    save_fig(gcf,fig_path,sprintf('Uniform_env_%sbins',num2str(numBins(b))))
end
