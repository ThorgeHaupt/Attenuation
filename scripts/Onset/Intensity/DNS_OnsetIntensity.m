%% DNS_OnsIntensity
%goal of this script is to sort the onsets according to the intensitiy in
...
%lower bound
lo_bound = [0 8 24 32 40];

%upper bound
up_bound = [72 80 88 96 104];
                ...
                for bw = 1:length(bin_width)
                    ...
                    for o = 1:length(ons_idx)
                        ...
                        if ons_idx(o)+5 < EEG.pnts
                            ons_env(1,o) = max(menv_norm(ons_idx(o)-1:ons_idx(o)+5));
                        else
...
%                     % Convert dB edges to linear scale by taking 10^(binEdges_dB/20)
                    binEdges_linear = binEdges_dB;%10.^(binEdges_dB / 20);
                    ...
                    [counts, ~, binIndices] = histcounts(ons_env, binEdges_normalized);
%                     
                    figure
...
DNS_Intensity.t = 'Grid search but with linear scaling';

save('DNS_Intensity_linearbins.mat','-struct','DNS_Intensity')

%% start plotting i guess
%% find the best prediction accuracy

fig_path = '\\daten.w2kroot.uni-oldenburg.de\home\lorf0331\Documents\MATLAB\Project\DNS_exploration\figures\Intensity\';
...
ylabel('Count (Samples)')
xlabel('Onset Intensity')
title('AB ons Grid Counts')
set(gca,'FontSize',14,'view',[90 -90])
box off

% save_fig(gcf,fig_path,'DNS_Intensity_Counts_log_envopt')
...
ylabel(t,'Density Bins (dB)','FontSize',16);
...
% save_fig(gcf,fig_path,'DNS_Intensity_Weights_log_envopt')
...
for k = 1:length(task)
    ons_pred = squeeze(mean(ons_preds.reg(:,k,:), 3));
...
%     save_fig(gcf,fig_path,sprintf('DNS_Intensity_Prediction_%s',task{k}))
end
...
%     save_fig(gcf,fig_path,sprintf('DNS_Intensity_Weights_%s',task{k}))
    
end



%% plot these bitches per participant

temp_parti = squeeze(mean(reg,[2 6]));
    

for s = 1:length(sbj)
    temp_dat = squeeze(temp_parti(s,:,:,:));
   
%     [rows, cols,pages] = ind2sub(size(temp_dat),find(temp_dat == max(temp_dat,[],'all')));    
%     
%     
%     %choose the scarcest model 
...
%     end
    ...
    reg_best(s)= temp_dat(rows,cols,pages);
    ...
    counts = squeeze(cat(3,sav_count{s,:,rows, cols, pages}));
    counts = sum(counts,2);
    ...
    set(gca,'FontSize',14,'view',[90 -90])
    box off
    %% plot the corresponding weights
%     figure
%     set(gcf,'position',fig_pos)
    %plot the weights
%     t = tiledlayout(2,2)
    weight_temp = [];
    for k = 1:length(task)
        weight_temp(:,:,:,:,k) = squeeze(cat(4,mlpt_weight{s,k,rows,cols,pages}));
    end
    
    plot_dat = squeeze(mean(weight_temp,[3 4 5]))';
    z_weight(:,:,s) = zscore(plot_dat,[],'all');
    nexttile
    y_val = [];
    for i =1:size(plot_dat,2)
        plot(trf_time,plot_dat(:,i)+(i*14),'Color',[0 0.4470 0.7410],'linew',2)
%         y_val(i) = mean(plot_dat(1:10,i)+(i*14));
    ...
    grid on
    title(sprintf('Model Weights Participant %s',sbj{s}))
%     set(gca,'FontSize',16,'YTick',round(y_val),'YtickLabel',binEdges_dB(2:end))
    box off
...
%     box off
%     
%     title(t,['DNS based TRF' num2str(best_width)],'FontSize',28)
    set(gca,'FontSize',16)
    
    
    
%     save_fig(gcf,fig_path,sprintf('DNS_Intensity_Weights_bin_width%s',num2str(a)))

    
    
end

%% plot the z-scored results
figure
plot_dat = squeeze(mean(z_weight,3));
for i =1:size(plot_dat,2)
    plot(trf_time,plot_dat(:,i)+(i*14),'Color',[0 0.4470 0.7410],'linew',2)
    %         y_val(i) = mean(plot_dat(1:10,i)+(i*14));
    hold on
...
%% loop through all the bin widths to look at the weights
temp_parti = squeeze(mean(reg,[1 2 6]));

for b = 1:length(bin_width)
    temp_dat = squeeze(temp_parti(:,:,b));
   
    [rows, cols] = ind2sub(size(temp_dat),find(temp_dat == max(temp_dat,[],'all')));    
...
%         y_val(i) = mean(plot_dat(1:10,i)+(i*14));
    ...
    grid on
    title(sprintf('Model Weights width %d',b))
%     set(gca,'FontSize',16,'YTick',round(y_val),'YtickLabel',binEdges_dB(2:end))
    box off
